...
const animateOnScroll = () => {
    const elements = document.querySelectorAll('.section-header, .game-card, .featured-game');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    style.textContent = `
        .section-header, .game-card, .featured-game {
            opacity: 0;
            transform: translateY(20px);
            transition: opacity 0.6s ease-out, transform 0.6s ease-out;
        }
        .section-header.animate, .game-card.animate, .featured-game.animate {
            opacity: 1;
